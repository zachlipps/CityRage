...
class CurrentUser extends React.Component {
  ...
  /*
  *   this is just returning a style object, 
  *   but it could just as easily return a className
  *   for styling in the css file
  */
  generatePlayerStyle(playerObj, gameData) {
    let playerStyle = {};
    if (playerObj.stats.health <= 0) playerStyle.textDecoration = 'line-through'
    return playerStyle
  }
    ...
    return (
      <div className="CurrentUser">
        ...
        <div className="CurrentUser--identification">
          ...
          {map(this.props.playersOnline, (item, key) => <div style = {this.generatePlayerStyle(item)} key={item.uid}>{this.generatePlayerIcon(item, this.props.game)} {item.displayName} {item.stats.health} {item.stats.energy} {item.stats.points}</div>)}
